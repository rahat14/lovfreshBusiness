...
public class OrderDetailPickupPopup extends AppCompatActivity implements ResponseListener, View.OnClickListener {
    ...
    private String mobileno = "", status = "", flag = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.order_detail_popup);
        getWindow().setLayout(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        getWindow().setBackgroundDrawable(new ColorDrawable(android.graphics.Color.TRANSPARENT));
        this.setFinishOnTouchOutside(false);
        ActivityCompat.requestPermissions(OrderDetailPickupPopup.this, PERMISSIONS, 112);
    ...
    private void readyToPickupStatusApi() {
        if (ConnectionUtil.isInternetOn(this)) {
            progressDialogUtil.showDialog();
            RequestHelper.patchRequest(NetworkHelper.REQ_CODE_READY_TO_PICKUP_STATUS, OrderDetailPickupPopup.this, Urls.ORDER_URL + orderId + "/", new NetworkHelper(this).selectAcceptJson("packed"), this);
        } else {
            BaseUtility.toastMsg(this, getString(R.string.no_internet_connection));
        }
    }

    private void confirmToPickupStatusApi() {
        if (ConnectionUtil.isInternetOn(this)) {
            progressDialogUtil.showDialog();
            RequestHelper.patchRequest(NetworkHelper.REQ_CODE_CONFIRM_TO_PICKUP_STATUS, OrderDetailPickupPopup.this, Urls.ORDER_URL + orderId + "/", new NetworkHelper(this).selectAcceptJson("completed"), this);
        } else {
            BaseUtility.toastMsg(this, getString(R.string.no_internet_connection));
        }
    }
        ...
        switch (requestCode) {
            case NetworkHelper.REQ_CODE_GET_ORDER_DETAIL:
                ...
                if (orderModel != null) {
                    ...
                    tvPaymentType.setText("Credit Card");
                    ...
                    if (orderModel.getStatus().equals("accepted")) {
                        swipeReady.setText("Ready to Pickup");
                        swipeReady.setVisibility(View.VISIBLE);
                        swipeConfirm.setVisibility(View.GONE);
                    } else if (orderModel.getStatus().equals("packed")) {
                        swipeConfirm.setText("Confirm to Pickup");
                        swipeConfirm.setVisibility(View.VISIBLE);
                        swipeReady.setVisibility(View.GONE);
                    } else if (orderModel.getStatus().equals("completed")) {
                        swipeReady.setVisibility(View.GONE);
                        swipeConfirm.setVisibility(View.GONE);
                    } else if (orderModel.getStatus().equals("created")) {
                        swipeReady.setVisibility(View.GONE);
                        swipeConfirm.setVisibility(View.GONE);
                    } else if (orderModel.getStatus().equals("rejected")) {
                ...
                break;
            case NetworkHelper.REQ_CODE_READY_TO_PICKUP_STATUS:
                progressDialogUtil.dismissDialog();
                Log.e(TAG, json.toString());
                getOrderDetailApi();
                swipeConfirm.setText("Confirm to Pickup");
                ...
                swipeConfirm.setVisibility(View.VISIBLE);
                break;
            case NetworkHelper.REQ_CODE_CONFIRM_TO_PICKUP_STATUS:
                progressDialogUtil.dismissDialog();
                Log.e(TAG, json.toString());
                getOrderDetailApi();
                swipeConfirm.setVisibility(View.GONE);
    ...
    private void selectStatusBtn() {
        if (btnChangeStatus.getText().toString().equals("Ready to Pickup")) {
            readyToPickupStatusApi();
        } else {
            confirmToPickupStatusApi();
        }
    ...
    public void Fileview(String invoicePdf, String order_no) {
        ...
        if (!hasPermissions(OrderDetailPickupPopup.this, PERMISSIONS)) {
            ...
            try {
                ...
                Toast.makeText(OrderDetailPickupPopup.this, "No Application available to view PDF", Toast.LENGTH_SHORT).show();
            }
        }
    ...
    private class DownloadFile extends AsyncTask<String, Void, Void> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            progressDialogUtil = new ProgressDialogUtil(OrderDetailPickupPopup.this);
            progressDialogUtil.showDialog();
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            Toast.makeText(OrderDetailPickupPopup.this, "Download Complete", Toast.LENGTH_SHORT).show();
            progressDialogUtil.dismissDialog();
        }
    }
}