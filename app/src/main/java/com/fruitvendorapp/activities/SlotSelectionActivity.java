...
public class SlotSelectionActivity extends AppCompatActivity implements View.OnClickListener, ItemSelectInterface, ResponseListener, JsonArrayResponseListener, ItemSelectTimeListInterface, ItemSelectTimeInterface {
    ...
    public void onSuccess(int requestCode, JSONObject json) {
        switch (requestCode) {
            ...
            case NetworkHelper.REQ_CODE_POST_SLOT_DATE:
                ...
                try {
                    if (json.has("mesage")) {
                        BaseUtility.toastMsg(SlotSelectionActivity.this, json.optString("mesage"));
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
